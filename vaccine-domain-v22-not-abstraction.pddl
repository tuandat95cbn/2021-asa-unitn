...
(define (domain vaccine-domain-v22)
	...
	(:action load
		...
		:precondition (and
			...
			(or (state ?location) (province ?location) (region ?location) (district ?location))
			(at ?vehicle ?location)
	...
	(:action un-load
		:parameters (?vehicle ?vaccine-box ?location)
		:precondition (and (or (plane ?vehicle) (truck ?vehicle) (drone ?vehicle))
			(vaccine-box ?vaccine-box)
			(or (state ?location) (province ?location) (region ?location) (district ?location))
			(at ?vehicle ?location)
			(in ?vaccine-box ?vehicle)
	...
	(:action move
		...
		:precondition (and
			(or (state ?location-1) (province ?location-1) (region ?location-1) (district ?location-1))
			(or (state ?location-2) (province ?location-2) (region ?location-2) (district ?location-2))
